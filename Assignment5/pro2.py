
total_girls=3
total_boys=3

def is_valid(g_left,b_left):
    g_right=total_girls-g_left
    b_right=total_boys-b_left

    if g_left<0 or b_left<0:
        return False
    if g_left>total_girls or b_left>total_boys:
        return False
    
    #left side condition
    if g_left>0 and b_left>g_left:
        return False
    
    #right side condition
    if g_right>0 and b_right>g_right:
        return False
    
    return True

def get_next_state(state):
    g_left=state[0]
    b_left=state[1]
    boat=state[2]

    next_states=[]
    moves=[(1,0),(2,0),(0,1),(0,2),(1,1)]  # 1G 0B, 2G 0B, 0G 1B, 0G 2B, 1G 1B

    for move in moves:
        g_move=move[0]
        b_move=move[1]

        #if boat on left
        if boat==0:
            new_g=g_left-g_move  #we subtract because we are moving from left to right
            new_b=b_left-b_move
            new_boat=1
        else: #boat on right
            new_g=g_left+g_move  #we add because we are moving from right to left
            new_b=b_left+b_move
            new_boat=0

        if is_valid(new_g,new_b):
            next_states.append((new_g,new_b,new_boat))

    return next_states

def depth_limited_search(state,goal,limit,path,visited):
    explored_states = 1
    if state==goal:
        return path, explored_states
    
    if limit==0:
        return None, explored_states
    
    visited.add(state)
    next_states=get_next_state(state)

    for next_s in next_states:
        if next_s not in visited:
            result, explored = depth_limited_search(next_s,goal,limit-1,path+[next_s],visited)
            explored_states += explored
            if result is not None:
                return result, explored_states
            
    visited.remove(state)
    return None, explored_states


def iterative_deepening_search(initial,goal):
    depth=0
    total_explored = 0
    while True:
        visited=set()
        result, explored = depth_limited_search(initial,goal,depth,[initial],visited)
        total_explored += explored
        if result is not None:
            return result, total_explored
        
        depth+=1


initial_state=(3,3,0)
goal_state=(0,0,1)

print("Depth Limited Search with limit = 3")
result, explored = depth_limited_search(initial_state, goal_state, 3, [initial_state], set())

if result is None:
    print("No solution found within depth 3")
else:
    for step in result:
        print(step)
print(f"Explored states: {explored}")


print("\nIterative Deepening Search")
solution, explored_ids = iterative_deepening_search(initial_state, goal_state)

print("Optimal Solution:")
for step in solution:
    print(step)
print(f"Explored states: {explored_ids}")

print("\n--- Comparison ---")
print("Depth-Limited Search (DLS):")
print(f"  - Explored States (limit=3): {explored}")
print("  - Time Complexity: O(b^d), where b is branching factor and d is depth limit.")
print("  - DLS is not complete and not optimal if the solution is deeper than the limit.")

print("\nIterative Deepening Search (IDS):")
print(f"  - Explored States: {explored_ids}")
print("  - Time Complexity: O(b^d), where b is branching factor and d is the depth of the solution.")
print("  - IDS is complete and optimal like Breadth-First Search, but with the memory benefits of Depth-First Search.")
